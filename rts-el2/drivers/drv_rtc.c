...
static rt_err_t set_rtc_time_stamp(time_t time_stamp)
{
    ...
    HAL_RTCEx_BKUPWrite(&RTC_Handler, RTC_BKP_DR10, RTC_DateStruct.Year);
    HAL_RTCEx_BKUPWrite(&RTC_Handler, RTC_BKP_DR11, RTC_DateStruct.Month);
    HAL_RTCEx_BKUPWrite(&RTC_Handler, RTC_BKP_DR12, RTC_DateStruct.Date);
    HAL_RTCEx_BKUPWrite(&RTC_Handler, RTC_BKP_DR13, RTC_DateStruct.WeekDay);
...
static rt_err_t rt_rtc_config(struct rt_device *dev)
{
    ...
    if (HAL_RTCEx_BKUPRead(&RTC_Handler, RTC_BKP_DR1) != BKUP_REG_DATA) {
        ...
        DateToUpdate.WeekDay = HAL_RTCEx_BKUPRead(&RTC_Handler, RTC_BKP_DR13);
        if (HAL_RTC_SetDate(&RTC_Handler, &DateToUpdate, RTC_FORMAT_BIN) != HAL_OK) {
            Error_Handler();
        }
    }
